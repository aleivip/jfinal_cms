...
null;"user"==("undefined"===typeof c?"user":c)?0!=a.urlParam("config.json.template")?(c="./scripts/"+a.urlParam("config.json.template"),userconfig=a.urlParam("config.json.template")):(c="./scripts/filemanager.config.json.template.js",userconfig="filemanager.config.json.template.js"):c="./scripts/filemanager.config.json.template.js.default";a.ajax({async:!1,url:c,dataType:"json",cache:!1,success:function(a){g=a}});return g},R=v("default"),f=v();null!==f&&delete f.version;f=a.extend({},R,f);if(f.options.logger)var A=(new Date).getTime();HEAD_included_files=[];loadCSS=function(c){if(-1==
